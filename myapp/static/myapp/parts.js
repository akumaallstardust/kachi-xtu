...
class post_moblie {
  constructor(
    ...
    this.listed_post_icon.href = site_url_base + `user/${this.user_id}/`;
    ...
    this.listed_post_username.href = site_url_base + `user/${this.user_id}/`;
    ...
    this.post_title.href = site_url_base + `post/${this.content_id}/`;
  ...
  post_comment(parent_comment_id = 0, comment_content) {
    if (user_id <= 0) {
      location.href = site_url_base + `signup/`;
    }
    if (comment_content != "") {
      ...
      return fetch(site_url_base + "add_comment/", {
        method: "post",
        headers: {
    ...
    static child_comment = class {
      constructor(parent_comment_box, comment_datas) {
        ...
        comment_username_text_link.href =
          site_url_base + `user/${comment_user_id}/`;
  ...
  open_comment() {
    return new Promise((resolve, reject) => {
      if (this.discussion_box_open_flag == false) {
        ...
        fetch(site_url_base + "get_discussion_data/", {
          method: "post",
          headers: {
            "Content-Type": "application/json", //JSON形式のデータのヘッダー
            "X-CSRFToken": csrftoken,
                ...
                this.post_comment(0, this.add_comment_textarea.value).then(
                  (data) => {
                    if (data["result"] == "success") {
                      ...
                      this.comment_id_list[this.comment_id_list.length] = String(
                        data["comment_id"]
                      );
                      ...
                      this.parent_comment_list.push(
                        new post_moblie.parent_comment(
                          this.content_id,
                          {
                            ...
                            user_id: Number(my_user_id_base),
                            username: username,
      ...
      if (this.already_viewed_flag == false) {
        ...
        if (user_id >= 1) {
          ...
          call_by_view_request.open(
            "POST",
            site_url_base + "view_count_doubleplus/",
            true
          );
  ...
  add_review(change____) {
    if (my_user_id_base >= 1) {
      let data = {
        change: change____,
        user_id: String(my_user_id_base),
        content_id: String(this.content_id),
        session_id_1: session_id_1,
        session_id_2: session_id_2,
      };
      fetch(site_url_base + "review_post/", {
        method: "post",
        headers: {
          "Content-Type": "application/json", //JSON形式のデータのヘッダー
          "X-CSRFToken": csrftoken,
